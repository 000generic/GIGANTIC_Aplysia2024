...
input_reports = open( 'output/4-list-blast-reports-Lottia', 'r' )
output_clusters = open( 'output/5-all-species-all-clusters-all-homologs-Lottia', 'w' )
output_map = open( 'output/5-map-final-clusterids-to-initial-cluster-ids-Lottia', 'w' )

# output/Lottia-granulata_X_Lottia-granulata
...
for next_gspp in gspp_gene_hits.keys(): # read through species in dictionary
    ...
    for next_gene in gspp_gene_hits[ next_gspp ]: # read through genes for species in dictionary
        ...
        previous_clusterids = []
        counter = counter + 1
        new_cluster = 'initial_Lottia_homolog_cluster_' + str( counter )
        ...
        for next_hit in gspp_gene_hits[ next_gspp ][ next_gene ]: # read through blast report hits per gene per species
            ...
            if len( next_hit.split( 'Lottia' ) ) > 1:
        ...
        for next_cluster in all_previous_clusters:
            if next_cluster == new_cluster: # protect new_cluster just in case
                ...
                for next_gene in clusters_genes[ next_cluster ]:
                    if len( next_gene.split( 'Lottia' ) ) > 1:
...
for next_cluster in clusters_genes:
    ...
    final_cluster = 'AAAB_cluster_Lottia_' + str( counter )
