...
input_reports = open( 'output/4-list-blast-reports-Homo', 'r' )
output_clusters = open( 'output/5-all-species-all-clusters-all-homologs-Homo', 'w' )
output_map = open( 'output/5-map-final-clusterids-to-initial-cluster-ids-Homo', 'w' )

# output/Homo-granulata_X_Homo-granulata
...
for next_gspp in gspp_gene_hits.keys(): # read through species in dictionary
    ...
    for next_gene in gspp_gene_hits[ next_gspp ]: # read through genes for species in dictionary
        ...
        previous_clusterids = []
        counter = counter + 1
        new_cluster = 'initial_Homo_homolog_cluster_' + str( counter )
...
for next_cluster in clusters_genes:
    ...
    final_cluster = 'AAAB_cluster_Homo_' + str( counter )
