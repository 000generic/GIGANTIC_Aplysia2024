...
output_counts = open( 'output/3-orthofinder-orthogroups-gains-losses-per-species-tree-node-RELATIVE-TO-APLYSIA', 'w' )
...
for next_clade in sorted( clades_orthogroups_total.keys() ):
    ...
    if next_clade != 'C37':
        ...
        for next_orthogroup in clade_orthogroups:
            if next_orthogroup in ancestor_orthogroups:
                ...
                if next_orthogroup in aplysia_orthogroups:
        ...
        for next_orthogroup in ancestor_orthogroups:
            if next_orthogroup not in clade_orthogroups:
                ...
                if next_orthogroup not in aplysia_orthogroups:
    ...
    output = next_clade + '\t' + clade_name  + '\t' + total_orthogroups + '\t' + str( conserved_orthogroups ) + '\t' + str( gained_orthogroups ) + '\t' + str( lost_orthogroups ) + '\t' + str( aplysia_conserved ) + '\t' + str( aplysia_gained ) + '\t' + str( aplysia_lost ) + '\n'
    output_counts.write( output )
